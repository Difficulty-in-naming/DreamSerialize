...
using DreamSerialize.Utility.System.Linq.Expressions;

namespace DreamSerialize.Utility
{

    public static class ExpressionVisitionUtility
    {
        public static Expression Visit(Expression node)
        {
            if (node != null)
            {
        ...
        public static Expression VisitBlock(this BlockExpressionUtility node)
        {
    ...
    public class BlockExpressionUtility : Expression
    {
        /// <summary>
        ...
        internal BlockExpressionUtility()
        {
        }
        ...
        /// </summary>
        protected override Expression Accept(ExpressionVisitor node)
        {
            int count = node.ExpressionCount;
            Expression[] nodes = null;
            for (int i = 0; i < count; i++)
    ...
    public static class ExpressionUtility
    {
            ...
            switch (expression.NodeType)
            {
                case ExpressionType.MemberAccess:
                    ...
                    if (member.MemberType != MemberTypes.Property || ((PropertyInfo)member).CanRead)
                        break;
                    throw new ArgumentException("参数必须可读" + paramName);
        ...
        private static void RequiresCanRead(IEnumerable<Expression> items, string paramName)
        {
            if (items == null)
                return;
            IList<Expression> expressionList = items as IList<Expression>;
        ...
        public static BlockExpression Block(Type type, IEnumerable<ParameterExpression> variables, IEnumerable<Expression> expressions)
        {
            ...
            if (AreEquivalent(type, last.Type))
            {
